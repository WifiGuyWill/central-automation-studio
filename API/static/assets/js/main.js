	...
	$.when(updateInventory()).then(function () {
		...
		$.each(csvData, function() {
			...
			if(!this["SITE"]) {
				...
				var siteInitials =  this["SITE"].match(/\b(\w)/g).join('');
				var device = findDeviceInInventory(currentSerial);
				if  (!device) {
					logError("Unable to find device " +  currentSerial + " in the device inventory");
					apiErrorCount++;
					...
					// grab AP number - sequential for each site
					var apNumber = renamingCounters[siteInitials];
					if  (!apNumber) renamingCounters[siteInitials] = 1;
					apNumber = renamingCounters[siteInitials];
					renamingCounters[siteInitials] = apNumber + 1;
					if (apNumber < 10) {
						newHostname = siteInitials+"-"+model+"-00"+apNumber;
					} else if (apNumber < 100) { 
						newHostname = siteInitials+"-"+model+"-0"+apNumber;
					} else {
						newHostname = siteInitials+"-"+model+"-"+apNumber;
					}
					magicNames[currentSerial] = newHostname;  // store incase of enhanced workflow requiring it.
					console.log(magicNames[currentSerial])
					// if AP then get AP settings
					var settings = {
						"url": api_url + "/tools/getCommand",
						"method": "POST",
		...
		$.when(licenseDevices()).then(function () {
			...
			$.when(moveDevicesToGroup()).then(function () {
				if (apiErrorCount != 0) {
					...
					Swal.fire({
					  title: 'Automation Failure',
					  text: 'Some or all devices failed to be added, licensed and move into a group',
					  icon: 'error'
					});
				} else {
					Swal.fire({
